...
Konfiguracja znajduje się w pliku \texttt{Doxyfile}. Najważniejsze parametry:
...
\texttt{HTML\_OUTPUT = html} & Katalog wynikowy HTML. \\
\texttt{GENERATE\_LATEX = YES} & Generowanie dokumentacji LaTeX. \\
\texttt{LATEX\_OUTPUT = latex} & Katalog wynikowy LaTeX. \\
...
Dokumentacja HTML jest dostępna w \texttt{html/index.html}. Dokumentacja LaTeX
jest generowana w katalogu \texttt{latex}. Plik PDF dokumentacji programistycznej
...
Wynikowy plik to \texttt{latex/refman.pdf}.
...
Wyniki są zapisywane w katalogu \texttt{uml}. Najważniejsze pliki:
\begin{itemize}[noitemsep]
    \item \texttt{uml/sudoku\_class\_diagram.puml},
    \item \texttt{uml/sudoku\_class\_diagram.svg},
    \item \texttt{uml/sudoku\_class\_diagram.png}.
\end{itemize}
